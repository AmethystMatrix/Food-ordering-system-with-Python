...
list_services = []                 #Variable List of other services, names + prices.O
...
def def_full_file_reader():                                                                         #mustafa
    ...
    while i <= (len(list_drinks) - 1):
        if 'RS' in list_drinks[i]:
            list_drinks[i] = str(list_drinks[i][:list_drinks[i].index('RS') - 1]) + ' ' * (20 - (list_drinks[i].index('RS') - 1)) + str(list_drinks[i][list_drinks[i].index('RS'):])
        i += 1
...
def def_file_sorter(): # Applying Sorting to the array to be sorted from A-Z ASC ((AND)) Extracting out prices after sorting and appending them to a prices array accordingly to a parrallel indexes
    ...
    while i < len(list_foods):
        list_item_price[i] = float(list_foods[i][int(list_foods[i].index("RS") + 3):]) # Extracting Out "RS" + [SPACE] from and cast out the string into an integer
        i += 1

    i = 0
    while i < len(list_drinks):
        list_item_price[40 + i] = float(list_drinks[i][int(list_drinks[i].index("RS") + 3):]) # Applying extraction on 40 and above items which are the drinks
        i += 1

    i = 0
    while i < len(list_services):
        list_item_price[80 + i] = float(list_services[i][int(list_services[i].index("RS") + 3):]) # Applying extraction on 80 and above items wich are Services
        i += 1
def_file_sorter()
...
def def_payment():
    while True:
        ...
        print(" " * 17 + "_" * 35 + "\n" + " " * 17 + "TOTAL PRICES:       RS " + str(round(total_price, 2))) #Calculate total bill
        report_new += "\n" + " " * 17 + "-" * 35 + "\n" + " " * 17 + "TOTAL PRICES:       RS " + str(
            round(total_price, 2)) + "\n" + " " * 17 + "*" * 35  #for saving payment in report file
        ...
        if (input_1 == 'P'):
            ...
            file_report.close()
            fd=input("Enter your feedback=\n")
            file_report = open('files' + navigator_symbol + 'report.fsd', 'a')  # Save it into a file
            file_report.write("\n                 Customers feedback="+fd+"\n                 ***********************************")
            file_report.close()
